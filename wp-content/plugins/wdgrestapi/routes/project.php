...
class WDGRESTAPI_Route_Project extends WDGRESTAPI_Route {
	...
	public function __construct() {
		...
		WDGRESTAPI_Route::register_wdg(
			'/project/(?P<id>\d+)/emails',
			WP_REST_Server::READABLE,
			array( $this, 'single_get_investment_emails'),
			array( 'id' => array( 'default' => 0 ) )
		);
	...
	public function single_get_declarations( WP_REST_Request $request ) {
		...
		if ( !empty( $project_id ) ) {
			...
			if ( !empty( $loaded_data ) && $this->is_data_for_current_client( $loaded_data ) ) {
				...
				$royalties_data = $project_item->get_declarations( ( $input_data_restricted_to_entity == '1' ), ( $input_data_with_links == '1' ) );
	...
	/**
	 * Retourne les emails envoyés en liaison avec un projet par son ID
	 * @param WP_REST_Request $request
	 * @return object
	 */
