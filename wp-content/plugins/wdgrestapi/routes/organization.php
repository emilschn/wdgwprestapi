...
class WDGRESTAPI_Route_Organization extends WDGRESTAPI_Route {
	public function __construct() {
		WDGRESTAPI_Route::register_wdg('/organizations', WP_REST_Server::READABLE, array( $this, 'list_get'));

		WDGRESTAPI_Route::register_wdg('/organizations/stats', WP_REST_Server::READABLE, array( $this, 'list_get_stats'));
		...
		WDGRESTAPI_Route::register_wdg('/organization/(?P<id>\d+)/virtual-iban', WP_REST_Server::READABLE, array( $this, 'single_get_virtual_iban'), array( 'token' => array( 'default' => 0 ) ));
	}
	...
	/**
	 * Retourne les investissements liés à une organisation (par l'ID de l'organisation)
	 * @param WP_REST_Request $request
	 * @return object
	 */
	public function single_get_investments(WP_REST_Request $request) {
		...
		if ( !empty( $organization_id ) ) {
			try {
				...
				if ( !empty( $loaded_data ) && $this->is_data_for_current_client( $loaded_data ) ) {
					$rois_data = $organization_item->get_investments();
