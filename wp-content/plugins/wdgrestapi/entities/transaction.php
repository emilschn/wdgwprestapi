...
class WDGRESTAPI_Entity_Transaction extends WDGRESTAPI_Entity
{
	...
	private static function list_get_by_entity_id($item_id, $is_legal_entity, $expanded = FALSE)
	{
		...
		$limit = $_GET['limit'];
		$userfilepath = __DIR__ . '/../files/transactions/' . $item_id . '_transactions.json';
		if (!file_exists(__DIR__ . '/../files/transactions/')) {
			mkdir(__DIR__ . '/../files/transactions/', 0755, true);
		}
		...
		if (empty($limit)) {
			$file = fopen($userfilepath, 'w');
			fputs($file, json_encode($results));
			fclose($file);
		}
			...
			foreach ($investments_by_user as $investment_item) {
				...
				if (!isset($previous_items_by_wedogood_entity_id['investment::' . $investment_item->id])) {
					...
					if (!empty($linked_p2p)) {
						...
						self::insert_item(
							$invest_datetime->format('Y-m-d H:i:s'),
							$investment_item->amount * 100,
							$item_id,
							$is_legal_entity,
							'',
							$orga_linked_id,
	...
	private static function insert_item(
		...
		$amount_in_cents,
		$sender_id,
		$is_sender_legal_entity,
		$sender_wallet_type,
		$recipient_id,
		$is_recipient_legal_entity,
		$recipient_wallet_type,
		$type,
		$status,
		$gateway_name,
		$gateway_mean_payment,
		$gateway_mean_payment_info,
